...
def calcola_conto(lista_piatti: list[Piatto]) -> float:
    totale = sum(piatto.get_prezzo() for piatto in lista_piatti)
    return totale

# output = funzione(input)
# def nome_funzione(input) -> output:
# def definisco la funzione
def raddoppia_numero(numero: int) -> int:
    return numero * 2

if __name__ == "__main__":
    # output = nome_funzione(input)
    numero_raddoppiato = raddoppia_numero(5)
    numero_raddoppiato = raddoppia_numero(8)
...
#     piatti_ordinati: list[Piatto] = [antipasto, primo, secondo, dolce]
...
#     # numero_raddoppiato = numero*2
#     # output = funzione(input)
#     # numero_raddoppiato = raddoppia_numero(numero)
